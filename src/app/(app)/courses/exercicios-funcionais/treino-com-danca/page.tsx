...
import { lessons, type Lesson } from '@/lib/treino-com-danca-lessons';
...
export default function TreinoComDancaPage() {
  const { user } = useAuth();
  ...
  const handleToggleComplete = (lessonId: string) => {
    if (!user || !db) return;
    const lessonKey = `danca-${lessonId}`;
    const userDocRef = doc(db, "users", user.uid);
    const isCompleted = completedLessons.includes(lessonKey);

    const updateData = { completedWorkoutLessons: isCompleted ? arrayRemove(lessonKey) : arrayUnion(lessonKey) };

    updateDoc(userDocRef, updateData).then(() => {
      if (isCompleted) {
        setCompletedLessons(prev => prev.filter(id => id !== lessonKey));
  ...
  const isCurrentLessonCompleted = completedLessons.includes(`danca-${selectedLesson.id}`);
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="mb-8 text-center">
            <h1 className="font-headline text-3xl font-bold tracking-tight">Treino com Dança</h1>
                    ...
                    <div className="space-y-2">
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2">
                                {completedLessons.includes(`danca-${lesson.id}`) && (
